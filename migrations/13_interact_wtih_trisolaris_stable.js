...
module.exports = async function (deployer, accounts) {
  ...
  // console.log(`**\nInital account balances: \nUSDT: ${usdt_balance}\nUSDC: ${usdc_balance}\nUSN: ${usn_balance}\nLP_Token: ${lp_token_balance}\n**`);
  // let liq0 = await assetRouter.deposit(swap_usdc_usdt_usn, ["10", "10", "10"], "0", "0", from_address, { from: from_address });
  // console.log("Tokens are deposited");
  // usdt_balance = await usdt_token.balanceOf(from_address);
  // usdc_balance = await usdc_token.balanceOf(from_address);
  // usn_balance = await usn_token.balanceOf(from_address);
  // lp_token_balance = await lp_token.balanceOf(from_address);
  // console.log(`**\nAccount balances after deposit: \nUSDT: ${usdt_balance}\nUSDC: ${usdc_balance}\nUSN: ${usn_balance}\nLP_Token: ${lp_token_balance}\n**`);
  // let userStake = await assetRouter.userStake(from_address, swap_usdc_usdt_usn);
  // console.log(`User stake: ${userStake.toString()}`);
  // await assetRouter.withdraw(swap_usdc_usdt_usn, userStake.toString(), ["0", "0", "0"], false, from_address, { from: from_address });
  ...
  // lp_token_balance = await lp_token.balanceOf(from_address);
  // console.log(`**\nAccount balances after withdrawal: \nUSDT: ${usdt_balance}\nUSDC: ${usdc_balance}\nUSN: ${usn_balance}\nLP_Token: ${lp_token_balance}\n**`);
  // let farmAddress = await farmFactory.Farms(swap_usdc_usdt_usn);
  // let farm = await Farm.at(farmAddress.toString());
  // let farmPid = await farm.pid();
  // console.log(farmPid.toString());
  // usdt_balance = await usdt_token.balanceOf(farmAddress.toString());
  // usdc_balance = await usdc_token.balanceOf(farmAddress.toString());
  // usn_balance = await usn_token.balanceOf(farmAddress.toString());
  // lp_token_balance = await lp_token.balanceOf(farmAddress.toString());
  // console.log(`**\nFarm balance after withdrawal: \nUSDT: ${usdt_balance}\nUSDC: ${usdc_balance}\nUSN: ${usn_balance}\nLP_Token: ${lp_token_balance}\n**`);
};
