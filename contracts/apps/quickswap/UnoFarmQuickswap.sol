...
contract UnoFarmQuickswap is Initializable, ReentrancyGuardUpgradeable {
	 ...
	 */
	IUniswapV2Router01 private constant quickswapRouter =
		...
		)
	{
		...
		if (amountA > 0 && amountB > 0) {
			(sentA, sentB, addedLiquidity) = quickswapRouter.addLiquidity(
				tokenA,
				tokenB,
				amountA,
				amountB,
	...
	function distribute(
		...
		if (tokenA != rewardToken) {
			...
			require(
				route[0] == rewardToken && route[route.length - 1] == tokenA,
		...
		if (tokenB != rewardToken) {
			...
			require(
				route[0] == rewardToken && route[route.length - 1] == tokenB,
		...
		(, , reward) = quickswapRouter.addLiquidity(
			tokenA,
			tokenB,
	...
	function collectFees(
		...
		if (feeInfo.feeTo != address(0)) {
			uint256 feeAmount = (IERC20Upgradeable(rewardToken).balanceOf(
				address(this)
			) * feeInfo.fee) / fractionMultiplier;
			if (feeAmount > 0) {
