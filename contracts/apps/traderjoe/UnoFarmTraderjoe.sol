...
contract UnoFarmTraderjoe is Initializable, ReentrancyGuardUpgradeable, IUnoFarmTraderjoe {
	...
	function distribute(
        ...
        IUniswapV2Router01 _traderjoeRouter = traderjoeRouter;
		...
		if (rewardTokenHalf > 0) {
			if (_tokenA != _rewardToken) {
				...
				_traderjoeRouter.swapExactTokensForTokens(rewardTokenHalf, swapInfos[0].amountOutMin, route, address(this), block.timestamp);
			}
			...
			if (_tokenB != _rewardToken) {
				...
				_traderjoeRouter.swapExactTokensForTokens(rewardTokenHalf, swapInfos[1].amountOutMin, route, address(this), block.timestamp);
			}
		}
		...
		if (rewarderTokenHalf > 0) {
			if (_tokenA != _rewarderToken) {
				...
				_traderjoeRouter.swapExactTokensForTokens(rewarderTokenHalf, swapInfos[2].amountOutMin, route, address(this), block.timestamp);
			}
			...
			if (_tokenB != _rewarderToken) {
				...
				_traderjoeRouter.swapExactTokensForTokens(rewarderTokenHalf, swapInfos[3].amountOutMin, route, address(this), block.timestamp);
			}
		}
		}

		(,,reward) = _traderjoeRouter.addLiquidity(_tokenA, _tokenB, IERC20(_tokenA).balanceOf(address(this)), IERC20(_tokenB).balanceOf(address(this)), swapInfos[0].amountOutMin, swapInfos[1].amountOutMin, address(this), block.timestamp);
