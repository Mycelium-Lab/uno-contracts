	...
	UUPSUpgradeable
{
	...
	receive() external payable {
		require(msg.sender == WAVAX, "ONLY_ACCEPT_WAVAX"); // only accept ETH via fallback from the WAVAX contract
	}
		...
		)
	{
		...
		if (amountLP > 0) {
			IBEP20(lpPair).safeTransferFrom(
				msg.sender,
				address(farm),
				amountLP
			);
		...
		if (amountA > 0) {
			IBEP20(farm.tokenA()).safeTransferFrom(
				msg.sender,
				address(farm),
				amountA
			);
		...
		if (amountB > 0) {
			IBEP20(farm.tokenB()).safeTransferFrom(
				msg.sender,
				address(farm),
				amountB
			);
		...
		)
	{
		...
		IBEP20(WAVAX).safeTransfer(address(farm), msg.value);
		if (tokenA == WAVAX) {
			if (amountToken > 0) {
				IBEP20(tokenB).safeTransferFrom(
					msg.sender,
					address(farm),
					amountToken
				);
			...
			if (amountToken > 0) {
				IBEP20(tokenA).safeTransferFrom(
					msg.sender,
					address(farm),
					amountToken
				);
		...
		returns (uint256 amountA, uint256 amountB)
	{
		...
		amountA =
			(amountLP *
				IBEP20(IUniswapV2Pair(lpPair).token0()).balanceOf(lpPair)) /
			totalSupply;
		amountB =
			(amountLP *
				IBEP20(IUniswapV2Pair(lpPair).token1()).balanceOf(lpPair)) /
			totalSupply;
	}
