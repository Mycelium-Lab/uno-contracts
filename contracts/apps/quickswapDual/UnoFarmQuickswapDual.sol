...
contract UnoFarmQuickswapDual is Initializable, ReentrancyGuardUpgradeable {
    ...
    function distribute(address[] calldata rewardTokenAToTokenARoute, address[] calldata rewardTokenAToTokenBRoute, address[] calldata rewardTokenBToTokenARoute, address[] calldata rewardTokenBToTokenBRoute, uint256[4] memory amountsOutMin) external onlyAssetRouter nonReentrant returns(uint256 reward){
        ...
        if (tokenA != rewardTokenA) {
            quickswapRouter.swapExactTokensForTokens(rewardTokenAHalf, amountsOutMin[0], rewardTokenAToTokenARoute, address(this), deadline);
        }

        if (tokenB != rewardTokenA) {
            quickswapRouter.swapExactTokensForTokens(rewardTokenAHalf, amountsOutMin[1], rewardTokenAToTokenBRoute, address(this), deadline);
        }
        }
        ...
        if (tokenA != rewardTokenB) {
            quickswapRouter.swapExactTokensForTokens(rewardTokenBHalf, amountsOutMin[2], rewardTokenBToTokenARoute, address(this), deadline);
        }

        if (tokenB != rewardTokenB) {
            quickswapRouter.swapExactTokensForTokens(rewardTokenBHalf, amountsOutMin[3], rewardTokenBToTokenBRoute, address(this), deadline);
        }
        }
