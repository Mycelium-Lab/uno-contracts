...
contract UnoAssetRouterQuickswapDual is Initializable, PausableUpgradeable, UUPSUpgradeable {
     ...
     */
    function withdrawSingleAsset(address lpStakingPool, uint256 amount, address token, bytes[2] calldata swapData, address recipient) external returns(uint256 amountToken, uint256 amountA, uint256 amountB){
        Farm farm = Farm(farmFactory.Farms(lpStakingPool));
        require(farm != Farm(address(0)),'FARM_NOT_EXISTS');
     ...
     * @return amountB - Token B dust sent to the {recipient}.
     */
    function withdrawSingleETH(address lpStakingPool,  uint256 amount, bytes[2] calldata swapData, address recipient) external returns(uint256 amountETH, uint256 amountA, uint256 amountB){
        Farm farm = Farm(farmFactory.Farms(lpStakingPool));
        require(farm != Farm(address(0)),'FARM_NOT_EXISTS');
