...
contract UnoFarmSushiswap is Initializable, ReentrancyGuardUpgradeable, IUnoFarmSushiswap {
    ...
    function distribute(
        ...
        IMiniChefV2 _MiniChef = MiniChef;
        _MiniChef.harvest(pid, address(this));

        address _tokenA = tokenA;
        address _tokenB = tokenB;
        IUniswapV2Router01 _sushiswapRouter = sushiswapRouter;
        ...
        if (rewardTokenHalf > 0) {
            if (_tokenA != _rewardToken) {
                ...
                _sushiswapRouter.swapExactTokensForTokens(rewardTokenHalf, swapInfos[0].amountOutMin, route, address(this), block.timestamp);
            }
            ...
            if (_tokenB != _rewardToken) {
                ...
                _sushiswapRouter.swapExactTokensForTokens(rewardTokenHalf, swapInfos[1].amountOutMin, route, address(this), block.timestamp);
            }
        }
        ...
        if (rewarderTokenHalf > 0) {
            if (_tokenA != _rewarderToken) {
                ...
                _sushiswapRouter.swapExactTokensForTokens(rewarderTokenHalf, swapInfos[2].amountOutMin, route, address(this), block.timestamp);
            }

            if (_tokenB != _rewarderToken) {
                ...
                _sushiswapRouter.swapExactTokensForTokens(rewarderTokenHalf, swapInfos[3].amountOutMin, route, address(this), block.timestamp);
            }
        }
        }

        (,,reward) = _sushiswapRouter.addLiquidity(_tokenA, _tokenB, IERC20(_tokenA).balanceOf(address(this)), IERC20(_tokenB).balanceOf(address(this)), swapInfos[0].amountOutMin + swapInfos[2].amountOutMin, swapInfos[1].amountOutMin + swapInfos[3].amountOutMin, address(this), block.timestamp);
        ...
        _MiniChef.deposit(pid, reward, address(this));
    }
